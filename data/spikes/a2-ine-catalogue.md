...
## Run 2 (same day, ~15 minutes later): INE blocked the runner

A second run from GitHub Actions got HTTP 403 on every `ine.pt` endpoint and a timeout on
`opc=2` (dados.gov.pt still fine). INE's bot protection evidently reacts to a recent 21 MB
catalogue pull from a cloud IP. Consequences for Phase B and the crosswalk:

- The run-1 evidence stands: the catalogue IS enumerable (`opc=2`, ~21 MB XML,
  `<indicator id=...>` entries with theme/subtheme; sample id 0000764).
- Fetch the INE catalogue **rarely and cache it in the repo**; never refetch per-indicator
  when the cached copy will do.
- Expect 403s from cloud IPs; retry later rather than harder, and keep requests sparse.
- Exact indicator count still unmeasured; take it from the cached catalogue when next
  fetched, not from a dedicated request.
